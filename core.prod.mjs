...
const getContext = (element, _containerState) => tryGetContext(element) || createContext$1(element);
...
const resolveContext = (context, hostElement) => {
...
const executeComponent = (rCtx, elCtx) => {
    ...
    newCtx.$cmpCtx$ = elCtx, newCtx.$slotCtx$ = null, invocatinContext.$subscriber$ = hostElement, 
    invocatinContext.$renderCtx$ = rCtx, componentQRL.$setContainer$(rCtx.$static$.$containerState$.$containerEl$);
    ...
    return safeCall((() => componentFn(props)), (jsxNode => waitOn.length > 0 ? Promise.all(waitOn).then((() => 1 & elCtx.$flags$ ? executeComponent(rCtx, elCtx) : {
        ...
        node: jsxNode,
        rCtx: newCtx
    }), (err => (handleError(err, hostElement), {
        node: SkipRender,
        rCtx: newCtx
    })));
};
...
const renderComponent = (rCtx, elCtx, flags) => {
    ...
    then(executeComponent(rCtx, elCtx), (res => {
        ...
        invocatinContext.$renderCtx$ = newCtx, justMounted && elCtx.$appendStyles$) {
        ...
        pause: () => (async (elmOrDoc, defaultParentJSON) => {
            ...
            const contexts = ((parent, predicate) => {
                ...
                const walker = parent.ownerDocument.createTreeWalker(parent, 129, {
                    acceptNode(node) {
                        if (isElement(el = node) && el.hasAttribute("q:container")) {
                            return 2;
                        }
                        var el;
            ...
            for (const elCtx of contexts) {
                ...
                if (elCtx.$id$ && elm.setAttribute("q:id", elCtx.$id$), isElement(elm) && listeners.length > 0) {
            ...
            const data = await _pauseFromContexts(contexts, containerState, (el => isNode(el) && isText(el) ? ((node, containerState) => {
...
const runSubscriber = async (watch, containerState, rctx) => (watch.$flags$, isResourceWatch(watch) ? runResource(watch, containerState) : runWatch(watch, containerState));
...
const runWatch = (watch, containerState, rctx) => {
    ...
    return safeCall((() => watchFn(opts)), (returnValue => {
        ...
        handleError(reason, hostElement);
    }));
};
...
const injectQContainer = containerEl => {
    directSetAttribute(containerEl, "q:version", "0.12.1"), directSetAttribute(containerEl, "q:container", "resumed"), 
