/**
 * @license
 * @builder.io/qwik/optimizer 2.0.0-0-dev+e03c429
...
var maybeThen = (promise, thenFn) => isPromise(promise) ? promise.then(thenFn, shouldNotError) : thenFn(promise);
...
var validateJSXNode = node => {
  if (qDev) {
    const {type: type, varProps: varProps, constProps: constProps, children: children} = node;
    invoke(void 0, (() => {
      ...
      if (children) {
        ...
        if (isBrowser && (isFunction(type) || constProps)) {
          const keys2 = {};
...
var useComputedQrl = qrl => {
  const {val: val, set: set, iCtx: iCtx, i: i} = useSequentialScope();
  if (val) {
    return val;
  }
  assertQrl(qrl);
  ...
  const task = new Task(26, i, iCtx.$hostElement$, qrl, signal, null);
  set(signal);
...
var isResourceReturn = obj => isObject(obj) && "resource" === (getProxyTarget(obj) || obj).__brand;
...
var getProxyTarget = obj => obj[QObjectTargetSymbol];
