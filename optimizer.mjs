/**
 * @license
 * @builder.io/qwik/optimizer 2.0.0-0-dev+9f8b88f
...
var vnode_diff = (container, jsxNode, vStartNode, scopedStyleIdPrefix) => {
  ...
  function descendContentToProject(children, host) {
    ...
    if (children.length) {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        const slotName = String(isJSXNode(child) && directGetPropsProxyProp(child.varProps, child.constProps, QSlot) || QDefaultSlot);
  ...
  function expectComponent(component) {
    ...
    if (componentMeta) {
      ...
      null != currentJSX.children && descendContentToProject(currentJSX.children, host);
    } else {
      vnode_insertBefore(journal, vParent, vNewNode = vnode_newVirtual(), vCurrent && getInsertBefore());
      ...
      const jsxOutput = executeComponent2(container, host, component$Host || container.rootVNode, component, getMergedProps(currentJSX.props));
...
var version = "2.0.0-0-dev+9f8b88f";
  ...
  set(target, prop, value) {
    ...
    if (prop in target) {
      ...
      if (newValue !== oldValue) {
        DEBUG2 && log("Store.set", oldValue, "->", newValue, pad("\n" + this.toString(), "  "));
        setNewValueAndTriggerEffects(prop, newValue, target, this);
      }
    } else {
      DEBUG2 && log("Store.set", "create property", newValue, pad("\n" + this.toString(), "  "));
      setNewValueAndTriggerEffects(prop, newValue, target, this);
    }
    return true;
  }
...
function serialize(serializationContext) {
  ...
  const writeObjectValue = (value, idx) => {
    ...
    if (fastSkipSerialize(value)) {
      ...
      let store = "" + $addRoot$(unwrapStore(value)) + " " + storeHandler.$flags$;
      ...
      writeString("" + value.$flags$ + " " + value.$index$ + " " + $addRoot$(value.$el$) + " " + $addRoot$(value.$effectDependencies$) + " " + qrlToString(serializationContext, value.$qrl$) + (null == value.$state$ ? "" : " " + $addRoot$(value.$state$)));
...
var frameworkType = obj => "object" === typeof obj && null !== obj && (obj instanceof Signal || obj instanceof Task2 || isJSXNode(obj)) || isQrl2(obj);
...
var canSerialize2 = value => {
  ...
  if ("object" === typeof value) {
    ...
    isStore(value) && (value = unwrapStore(value));
...
var directGetPropsProxyProp = (varProps, constProps, prop) => constProps && prop in constProps ? constProps[prop] : varProps[prop];

var getMergedProps = props => ({
  ...props[_VAR_PROPS],
  ...props[_CONST_PROPS],
...
var PropsProxyHandler = class {
  ...
  get(_, prop) {
    ...
    const value = directGetPropsProxyProp(this.$varProps$, this.$constProps$, prop);
