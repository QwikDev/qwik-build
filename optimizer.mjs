/**
 * @license
 * @builder.io/qwik/optimizer 2.0.0-0-dev+80086ce
...
function createPlugin(optimizerOptions = {}) {
  ...
  const resolveId = (ctx, id2, importerId, resolveOpts) => {
    if (id2.startsWith("\0")) {
      return;
    }
    const count2 = resolveIdCount++;
    const isServer2 = getIsServer(resolveOpts);
    debug(`resolveId(${count2})`, `begin ${id2} | ${isServer2 ? "server" : "client"} | ${importerId}`);
    ...
    if (devServer && id2.startsWith("/") && importerId?.endsWith(".html")) {
      const maybeQrl = decodeURIComponent(id2);
      maybeQrl.startsWith("/@fs/") ? id2 = maybeQrl.slice(5) : maybeQrl.startsWith(importerDir.slice(0, importerDir.indexOf("/", 1))) || (id2 = path.join(importerDir, maybeQrl));
    }
    id2.startsWith(".") && importerDir && (id2 = path.resolve(importerDir, id2));
    const parsedId = parseId(id2);
    const pathId = normalizePath(parsedId.pathId);
    let result;
    if (parentIds.get(pathId)) {
      debug(`resolveId(${count2}) Resolved already known ${pathId}`);
      result = {
      ...
      if (match) {
        ...
        return ctx.resolve(parentId, importerId, {
          skipSelf: true
        }).then((resolved => {
          if (resolved) {
            debug(`resolveId(${count2})`, `resolved to QRL ${name} of ${parentId}`);
          ...
          debug(`resolveId(${count2})`, `${parentId} does not exist`);
        }));
      }
      if (importerId) {
        const importerParentId = parentIds.get(importerId);
        if (importerParentId) {
          debug(`resolveId(${count2})`, `resolving via ${importerParentId}`);
    ...
    debug(`resolveId(${count2}) end`, result?.id || result);
    return result;
  };
  let loadCount = 0;
  const load = async (ctx, id2, loadOpts) => {
    ...
    if (id2 === `/${QWIK_CLIENT_MANIFEST_ID}`) {
      debug(`load(${count2})`, QWIK_CLIENT_MANIFEST_ID, opts.buildMode);
      return {
        moduleSideEffects: false,
    ...
    if (devServer && !outputs.has(id2)) {
      ...
      if (parentId) {
        debug(`load(${count2})`, "transforming QRL parent", parentId);
        await devServer.transformRequest(`/@fs${parentId}`);
        if (!outputs.has(id2)) {
          debug(`load(${count2})`, `QRL segment ${id2} not found in ${parentId}`);
          return null;
        }
      }
    }
    ...
    debug(`load(${count2})`, "Not a QRL or virtual module", id2);
    return null;
  };
  ...
  const transform = async function(ctx, code, id2, transformOpts = {}) {
    ...
    if (ext in TRANSFORM_EXTS || TRANSFORM_REGEX.test(pathId)) {
      ...
      debug(`transform(${count2})`, `Transforming ${id2} (for: ${isServer2 ? "server" : "client"}${strip ? ", strip" : ""})`);
      ...
      for (const mod of newOutput.modules) {
        if (mod !== module) {
          ...
          debug(`transform(${count2})`, `segment ${key}`, mod.segment.displayName);
    ...
    debug(`transform(${count2})`, "Not transforming", id2);
    return null;
  };
...
var vnode_diff = (container, jsxNode, vStartNode, scopedStyleIdPrefix) => {
  ...
  function expectComponent(component) {
    ...
    if (componentMeta) {
      ...
      if (!lookupKeysAreEqual) {
        vNewNode = retrieveChildWithKey(null, lookupKey);
        vNewNode ? vnode_insertBefore(journal, vParent, vNewNode, vCurrent) : insertNewInlineComponent();
        host = vNewNode;
      }
      if (host) {
        let componentHost = host;
        while (componentHost && (!vnode_isVirtualVNode(componentHost) || null === vnode_getProp(componentHost, OnRenderProp, null))) {
  ...
  function insertNewInlineComponent() {
    ...
    jsxValue.key && vnode_setProp(vNewNode, ELEMENT_KEY, jsxValue.key);
  }
...
var encode = url => encodeURIComponent(url).replaceAll("%2F", "/").replaceAll("%40", "@").replaceAll("%3A", ":").replaceAll("%5B", "[").replaceAll("%5D", "]").replaceAll("%2C", ",");
