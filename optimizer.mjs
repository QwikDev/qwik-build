/**
 * @license
 * @builder.io/qwik/optimizer 1.6.0-dev20240703040019
...
function generateManifestFromBundles(path2, hooks, injections, outputBundles, opts) {
  ...
  for (const [fileName, outputBundle] of Object.entries(outputBundles)) {
    ...
    const bundleFileName = path2.basename(fileName);
...
function createPlugin(optimizerOptions = {}) {
  ...
  const normalizeOptions = inputOpts => {
    ...
    if (!updatedOpts.csr) {
      if (Array.isArray(updatedOpts.input)) {
        ...
        opts.input = [ path2.resolve(srcDir, "entry.ssr") ];
      } else if ("client" === opts.target) {
        opts.input = [ path2.resolve(srcDir, "root") ];
        ...
        if ("object" === typeof updatedOpts.input) {
          for (const key in updatedOpts.input) {
            ...
            if (Object.hasOwnProperty.call(updatedOpts.input, key)) {
              ...
              const absolutePath = path2.resolve(opts.rootDir, relativePath);
          ...
          opts.input = [ path2.resolve(srcDir, "index.ts") ];
        }
      } else {
        opts.input = [];
      }
      opts.input = Array.isArray(opts.input) ? opts.input.reduce(((inputs, i) => {
        let input = i;
        i.startsWith("@") || i.startsWith("~") || i.startsWith("#") || (input = normalizePath(path2.resolve(opts.rootDir, i)));
  ...
  const resolveId = async (ctx, id2, importerId, resolveOpts) => {
    ...
    if (importerId) {
      ...
      if ("ssr" === opts.target && !isSSR && importerId.endsWith(".html") && server) {
        ...
        const isAbsoluteFile = id2.startsWith("/@fs/");
        isAbsoluteFile && (id2 = id2.slice(4));
        const match = /^([^?]*)\?_qrl_parent=(.*)/.exec(id2);
        if (match) {
          id2 = match[1];
          const parentId = id2.slice(0, id2.lastIndexOf("/") + 1) + match[2];
          isAbsoluteFile || (id2 = normalizePath(path2.join(dir, id2)));
      ...
      if (ext in RESOLVE_EXTS) {
        ...
        importeePathId = normalizePath(path2.resolve(dir, importeePathId));
  ...
  const load = async (ctx, id2, loadOpts) => {
    ...
    if (transformedModule) {
      ...
      if ("ssr" === opts.target) {
        const firstInput = Object.values(opts.input)[0];
        code = code.replace(/@qwik-client-manifest/g, normalizePath(path2.resolve(firstInput, QWIK_CLIENT_MANIFEST_ID)));
      }
      return {
        code: code,
        map: map,
  ...
  const createOutputAnalyzer = rollupBundle => {
    ...
    const generateManifest = async () => {
      ...
      const manifest = generateManifestFromBundles(path2, hooks, injections, rollupBundle, opts);
...
var encode = url => encodeURIComponent(url).replaceAll("%2F", "/");

function createSymbolMapper(base, opts, foundQrls) {
  return (symbolName, mapper, parent) => {
    ...
    const qrlPath = parentPath.startsWith(opts.rootDir) ? path.posix.normalize(path.relative(opts.rootDir, parentPath)) : `@fs${path.posix.normalize(parentPath)}`;
...
async function configureDevServer(base, server, opts, sys, path2, isClientDevOnly, clientDevInput, foundQrls) {
      ...
      if (src && currentHref && extensionSupportsImport) {
        ...
        if (urlSrc.origin === urlCurrent.origin) {
          ...
          if (fs.existsSync(publicImagePath)) {
            ...
            await fs.promises.copyFile(publicImagePath, path2.join(srcDir, "media", urlSrc.pathname));
...
function qwikVite(qwikViteOpts = {}) {
  ...
  async function loadQwikInsights(clientOutDir2) {
    ...
    if (fs.existsSync(path2)) {
      ...
      return JSON.parse(await fs.promises.readFile(path2, "utf-8"));
    }
    return null;
  }
  const api = {
    ...
    configureServer(server) {
      ...
      if (!qwikViteOpts.csr && devSsrServer) {
        const plugin = async () => {
          ...
          const sys = qwikPlugin.getSys();
          const path2 = qwikPlugin.getPath();
          await configureDevServer(basePathname, server, opts, sys, path2, isClientDevOnly, clientDevInput, qwikPlugin.foundQrls);
        };
