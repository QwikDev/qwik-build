...
globalThis.qwikOptimizer = function(module) {
  ...
  async function getSystem() {
    ...
    if ("node" === sysEnv || "bun" === sysEnv) {
      sys.dynamicImport = path2 => require(path2);
      sys.strictDynamicImport = path2 => import(path2);
      ...
      sys.strictDynamicImport = path2 => import(path2);
      sys.dynamicImport = async path2 => {
        const cjsRsp = await fetch(path2);
  ...
  function createPlugin(optimizerOptions = {}) {
    ...
    const load = async (ctx, id2, loadOpts) => {
      ...
      const isSSR = !!(null == loadOpts ? void 0 : loadOpts.ssr);
      if (opts.resolveQwikBuild && id2.endsWith(QWIK_BUILD_ID)) {
  ...
  function createSymbolMapper(base, opts, foundQrls) {
    return (symbolName, mapper, parent) => {
      ...
      const parentPath = import_path2.default.dirname(parent);
      const parentFile = import_path2.default.basename(parent);
      const qrlPath = parentPath.startsWith(opts.rootDir) ? import_path2.default.posix.normalize(import_path2.default.relative(opts.rootDir, parentPath)) : `@fs${import_path2.default.posix.normalize(parentPath)}`;
    ...
    const vitePluginPre = {
      ...
      async config(viteConfig, viteEnv) {
        ...
        if (!qwikViteOpts.csr) {
          ...
          clientDevInput = "string" === typeof (null == (_p = qwikViteOpts.client) ? void 0 : _p.devInput) ? path2.resolve(opts.rootDir, qwikViteOpts.client.devInput) : opts.srcDir ? path2.resolve(opts.srcDir, CLIENT_DEV_INPUT) : path2.resolve(opts.rootDir, "src", CLIENT_DEV_INPUT);
