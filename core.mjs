...
const maybeThenPassError = (promise, thenFn) => {
...
const renderNode = (node, rCtx, ssrCtx, stream, flags, beforeClose) => {
    ...
    if (typeof tagName === 'string') {
        ...
        const handleProp = (rawProp, value, isImmutable) => {
            ...
            if (isOnProp(rawProp)) {
                setEvent$1(elCtx.li, rawProp, value, undefined);
                return;
            }
            if (isSignal(value)) {
...
const isTask$1 = (task) => (task.$flags$ & TaskFlags.TASK) !== 0;
const isResourceTask$1 = (task) => (task.$flags$ & TaskFlags.RESOURCE) !== 0;
...
const runSubscriber2 = async (task, container, host) => {
